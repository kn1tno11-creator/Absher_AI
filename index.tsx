...
type View = 'LOGIN' | 'DASHBOARD' | 'VIOLATIONS' | 'PASSPORT' | 'APPOINTMENTS' | 'SETTINGS' | 'DOCUMENTS';
...
const MOCK_DATA = {
  ...
  sectors: [
      { id: 'traffic', nameEn: 'Traffic', nameAr: 'المرور', icon: 'fa-car' },
      { id: 'passport', nameEn: 'Passports', nameAr: 'الجوازات', icon: 'fa-passport' },
      { id: 'civil', nameEn: 'Civil Affairs', nameAr: 'الأحوال المدنية', icon: 'fa-id-card' },
  ],
  services: {
      'traffic': ['License Renewal', 'Vehicle Transfer', 'Plate Issuance'],
      'passport': ['Passport Renewal', 'Visa Issuance', 'Travel Permit'],
      'civil': ['ID Renewal', 'Birth Registration', 'Family Card']
  }
};
...
const processVoiceCommand = async (transcript: string, currentContext: any, lang: Language) => {
  ...
  const systemPrompt = `
    ...
    Current User: ${USER_NAME_EN}. View: ${currentContext.view}. Lang: ${lang}.
    Appointment State: ${JSON.stringify(currentContext.appointmentState || {})}.

    User Input: "${transcript}"

    Instructions:
    1. Determine Intent: LOGIN, NAVIGATE, BOOK_APPOINTMENT, FILL_FORM, SHOW_DOCS, GENERAL_QUERY.
    2. For APPOINTMENTS:
       - If user says "Book appointment for Traffic", output action: 'SET_APPOINTMENT_SECTOR', sector: 'traffic'.
       - If user says a service name (e.g. "License Renewal"), output action: 'SET_APPOINTMENT_SERVICE', service: 'License Renewal'.
       - If user says a date/time (e.g. "Tomorrow morning"), output action: 'SET_APPOINTMENT_TIME', time: 'Tomorrow 10:00 AM'.
    3. For NAVIGATION: action: 'NAVIGATE_[VIEW_NAME]'.
    4. For DOCUMENTS: action: 'NAVIGATE_DOCUMENTS'.
    5. Reply in the same language.

    JSON Schema:
    {
      "action": "string",
      "targetView": "string",
      "data": "object (e.g. {sector, service, time})",
...
const Sidebar = ({ currentView, setView, isHighContrast, lang }: any) => {
  const menuItems = [
    ...
    { id: 'DOCUMENTS', labelEn: 'Digital Documents', labelAr: 'وثائقي الرقمية', icon: 'fa-id-card' },
...
const DocumentsView = ({ isHighContrast, data, lang }: any) => {
    return (
        <div className="space-y-8 animate-fade-in">
             <div className="flex justify-between items-center">
                 <h1 className="text-2xl font-extrabold text-gray-900">{lang === 'ar-SA' ? 'وثائقي الرقمية' : 'Digital Documents'}</h1>
                 <button className="bg-absher-green text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2">
                     <i className="fas fa-qrcode"></i> {lang === 'ar-SA' ? 'عرض الكل' : 'View All QR'}
                 </button>
             </div>

             <div className="grid md:grid-cols-2 gap-8">
                 {/* National ID Card */}
                 <div className={`aspect-[1.586] rounded-3xl relative overflow-hidden shadow-2xl transition-transform hover:scale-[1.02] ${isHighContrast ? 'border-2 border-yellow-400' : ''}`}>
                      <div className="absolute inset-0 bg-gradient-to-br from-teal-800 to-emerald-600"></div>
                      ...
                      <div className="relative p-6 h-full flex flex-col justify-between text-white">
                          <div className="flex justify-between items-start">
                              <div className="flex gap-4 items-center">
                                  <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/0d/Flag_of_Saudi_Arabia.svg/1200px-Flag_of_Saudi_Arabia.svg.png" className="w-12 h-8 rounded shadow-sm object-cover" alt="flag"/>
                                  <div>
                                      <p className="text-[10px] uppercase opacity-70 tracking-widest">Kingdom of Saudi Arabia</p>
                                      <p className="font-bold text-lg">{lang === 'ar-SA' ? 'الهوية الوطنية' : 'National ID'}</p>
                                  </div>
                              </div>
                              <i className="fas fa-chip text-3xl opacity-50 text-yellow-300"></i>
                          </div>
                          <div className="flex gap-6 items-end">
                              <div className="w-24 h-28 bg-gray-300 rounded-lg shadow-inner overflow-hidden">
                              ...
                              <div className="space-y-2 flex-1">
                                  <div>
                                      ...
                                      <p className="font-bold text-lg leading-tight">{lang === 'ar-SA' ? USER_NAME_AR : USER_NAME_EN}</p>
                                  </div>
                                  <div className="flex justify-between">
                                      <div>
                                          <p className="text-[10px] uppercase opacity-60">ID Number</p>
                                          <p className="font-mono font-bold tracking-widest">{USER_ID}</p>
                                      </div>
                                      <div>
                                          <p className="text-[10px] uppercase opacity-60">DOB</p>
                                          <p className="font-mono font-bold">01/01/1980</p>
                                      </div>
                                  </div>
                              </div>
                          </div>
                      ...
                      <div className="relative p-6 h-full flex flex-col justify-between text-white">
                          <div className="flex justify-between items-start">
                              <div className="flex gap-4 items-center">
                                  ...
                                  <div>
                                      <p className="font-bold text-lg">{lang === 'ar-SA' ? 'رخصة القيادة' : 'Driving License'}</p>
                                      <p className="text-[10px] uppercase opacity-70 tracking-widest">Private</p>
                                  </div>
                              </div>
                              <div className="px-2 py-1 bg-green-500 rounded text-xs font-bold">VALID</div>
                          </div>
                          <div className="space-y-4">
                               <div className="grid grid-cols-2 gap-4">
                                   <div>
                                       <p className="text-[10px] uppercase opacity-60">License No</p>
                                       <p className="font-mono font-bold">DL-9928371</p>
                                   </div>
                                   <div>
                                       <p className="text-[10px] uppercase opacity-60">Expiry</p>
                                       <p className="font-mono font-bold">2028-05-15</p>
                                   </div>
                               </div>
                               <p className="text-[10px] opacity-50 w-full text-justify">
                                   Authorized to drive private vehicles within KSA and GCC countries.
                               </p>
                          </div>
                      </div>
                 </div>
...
const AppointmentsView = ({ isHighContrast, data, lang, appState, setAppState, onBook }: any) => {
    // Steps: 1-Sector, 2-Service, 3-Time, 4-Success
    const step = appState.step || 1;

    return (
        <div className="space-y-6 animate-fade-in h-full flex flex-col">
            <h1 className="text-2xl font-extrabold text-gray-900 mb-4">{lang === 'ar-SA' ? 'حجز موعد جديد' : 'Book New Appointment'}</h1>
            
            {/* Progress Bar */}
            <div className="flex items-center gap-2 mb-8">
                 {[1, 2, 3].map(s => (
                     <div key={s} className={`h-2 flex-1 rounded-full transition-all duration-500 ${step >= s ? 'bg-absher-green' : 'bg-gray-200'}`}></div>
                 ))}
            </div>

            <div className={`flex-1 rounded-3xl p-8 shadow-sm border overflow-y-auto ${isHighContrast ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                ...
                {step === 1 && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
                        {data.sectors.map((sec:any) => (
                            <button 
                                key={sec.id}
                                onClick={() => setAppState({...appState, step: 2, sector: sec.id})}
                                className={`p-8 rounded-2xl border-2 flex flex-col items-center gap-4 transition-all hover:border-absher-green hover:bg-absher-light/20 ${appState.sector === sec.id ? 'border-absher-green bg-absher-light/30' : 'border-gray-100'}`}
                            >
                                <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center text-2xl text-absher-green">
                                    <i className={`fas ${sec.icon}`}></i>
                                </div>
                                <span className="font-bold text-lg text-gray-800">{lang === 'ar-SA' ? sec.nameAr : sec.nameEn}</span>
                            </button>
                        ))}
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="space-y-4 animate-fade-in">
                        <button onClick={() => setAppState({...appState, step: 1})} className="text-gray-400 text-sm mb-4"><i className="fas fa-arrow-left"></i> Back</button>
                        <h3 className="font-bold text-lg mb-4">{lang === 'ar-SA' ? 'اختر الخدمة' : 'Select Service'}</h3>
                        <div className="grid grid-cols-1 gap-3">
                             {data.services[appState.sector]?.map((srv: string) => (
                                 <button 
                                    key={srv}
                                    onClick={() => setAppState({...appState, step: 3, service: srv})}
                                    className="p-4 rounded-xl border text-left hover:border-absher-green hover:bg-absher-light/10 font-medium"
                                 >
                                    {srv}
                                 </button>
                             ))}
                ...
                {step === 3 && (
                     <div className="space-y-6 animate-fade-in text-center py-10">
                         <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto text-blue-500 text-3xl mb-4">
                             <i className="far fa-clock"></i>
                         </div>
                         <h3 className="font-bold text-xl">{lang === 'ar-SA' ? 'الموعد المقترح' : 'Suggested Time'}</h3>
                         <p className="text-gray-500">Based on availability</p>
                         
                         <div className="inline-block border-2 border-absher-green rounded-2xl p-6 bg-white shadow-lg">
                             <p className="text-3xl font-black text-gray-800">10:00 AM</p>
                             <p className="text-gray-500 font-medium">Tomorrow, Oct 24</p>
                         </div>

                         <div className="flex justify-center gap-4 mt-8">
                             <button onClick={() => setAppState({...appState, step: 2})} className="px-6 py-3 rounded-xl border font-bold text-gray-500">Change</button>
                             <button onClick={onBook} className="px-8 py-3 rounded-xl bg-absher-dark text-white font-bold shadow-lg hover:bg-absher-green transform hover:scale-105 transition">
                                 {lang === 'ar-SA' ? 'تأكيد الحجز' : 'Confirm Booking'}
                             </button>
                         </div>
                     </div>
                )}
                ...
                {step === 4 && (
                    <div className="text-center py-20 animate-fade-in">
                        <div className="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 text-green-600 text-4xl">
                            <i className="fas fa-check"></i>
                        </div>
                        <h2 className="text-2xl font-bold text-gray-900 mb-2">{lang === 'ar-SA' ? 'تم حجز الموعد بنجاح' : 'Appointment Booked Successfully'}</h2>
                        <p className="text-gray-500 max-w-md mx-auto">{lang === 'ar-SA' ? 'تم إرسال تفاصيل الموعد إلى هاتفك الجوال.' : 'Appointment details have been sent to your registered mobile number.'}</p>
                        <button onClick={() => setAppState({step: 1})} className="mt-8 text-absher-green font-bold underline">Book Another</button>
                    </div>
                )}
            </div>
        </div>
...
function AbsherApp() {
  ...
  const [appointmentState, setAppointmentState] = useState<any>({ step: 1 });
  ...
  }, [currentView, mockData, lang, activeField, appointmentState]); 
  ...
  const handleVoiceInput = async (text: string) => {
    ...
    if (result.action === 'LOGIN') {
        ...
        speakText(lang === 'ar-SA' ? 'مرحباً بك في أبشر' : 'Welcome to Absher', lang);
    }
    ...
    if (result.action === 'NAVIGATE_DOCUMENTS') setCurrentView('DOCUMENTS');
    if (result.action === 'NAVIGATE_SETTINGS') setCurrentView('SETTINGS');
    
    // Booking Wizard Logic
    if (result.action.startsWith('SET_APPOINTMENT')) {
        setCurrentView('APPOINTMENTS');
        if (result.action === 'SET_APPOINTMENT_SECTOR' && result.data?.sector) {
    ...
    if (result.speechResponse && result.action !== 'LOGIN') {
  ...
  const handleBookingConfirm = () => {
      triggerVerification(() => {
          setAppointmentState({ step: 4 });
          const msg = lang === 'ar-SA' ? "تم حجز الموعد بنجاح." : "Appointment confirmed successfully.";
          setAssistantResponse(msg);
          speakText(msg, lang);
      });
  };
      ...
      {currentView === 'LOGIN' ? (
          ...
          <>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <div className="max-w-7xl mx-auto p-6 md:p-10 pb-32 h-full">
                    {currentView === 'DASHBOARD' && <DashboardView isHighContrast={isHighContrast} data={mockData} lang={lang} />}
                    {currentView === 'DOCUMENTS' && <DocumentsView isHighContrast={isHighContrast} data={mockData} lang={lang} />}
                    ...
                    {currentView === 'APPOINTMENTS' && <AppointmentsView isHighContrast={isHighContrast} data={mockData} lang={lang} appState={appointmentState} setAppState={setAppointmentState} onBook={handleBookingConfirm} />}
